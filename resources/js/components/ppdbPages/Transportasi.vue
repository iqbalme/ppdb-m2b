		...
		<div class="container-fluid">
			<div class="row">
				<div class="col-md-12">
						...
						<div class="row" v-if="!loading">
						<div class="col-md-12">
						<base-input type="text" label="" v-model="transportasi.value" :validatedClass="$v.transportasi.$error" placeholder="Masukkan transportasi di sini"></base-input>
...
export default {
    ...
    data() {
        return {
			...
			data: [],
			kolom: ['no', 'transportasi', {key:'aksi', label:''}],
        };
    },
    ...
    methods: {
		...
		editData(id){
			this.transportasi.value = this.data[id].transportasi;
			this.transportasi.id = this.data[id].id;
		},
		hapusData(id){
			if(this.data.length > 0){
				if(this.transportasi.id != null){
					this.data.splice(id,1);
				}
				$axios
				.delete('/transportasi/' + this.data[id].id)
				.then((response)=> {
		...
		simpanData(){
			if(this.transportasi.value != null){
			  $axios
				.post('/transportasi', { 'transportasi' : this.transportasi.value, 'id' : this.transportasi.id })
				.then((response)=> {
				  var res = response.data;
		...
		clearForm(){
			this.transportasi.value = null;
			this.transportasi.id = null;
			this.$v.$reset();
		},
		gettransportasi() {
			  $axios
				...
				.then((response)=> {
				  ...
				  for (var i = 0; i < items.length; i++) {
					this.data.push({
						id : items[i].id,
						transportasi: items[i].transportasi
					});
				  }
				})
				.catch(error => {
