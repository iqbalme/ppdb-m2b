		...
		<div class="container-fluid">
			<div class="row">
				<div class="col-md-12">
						...
						<div class="row" v-if="!loading">
						<div class="col-md-12">
						<base-input type="text" label="" v-model="pekerjaan.value" :validatedClass="$v.pekerjaan.$error" placeholder="Masukkan pekerjaan di sini"></base-input>
...
export default {
    ...
    data() {
        return {
			...
			data: [],
			kolom: ['no', 'pekerjaan', {key:'aksi', label:''}],
        };
    },
    ...
    methods: {
		...
		editData(id){
			this.pekerjaan.value = this.data[id].pekerjaan;
			this.pekerjaan.id = this.data[id].id;
		},
		hapusData(id){
			if(this.data.length > 0){
				if(this.pekerjaan.id != null){
					this.data.splice(id,1);
				}
				$axios
				.delete('/pekerjaan/' + this.data[id].id)
				.then((response)=> {
		...
		simpanData(){
			if(this.pekerjaan.value != null){
			  $axios
				.post('/pekerjaan', { 'pekerjaan' : this.pekerjaan.value, 'id' : this.pekerjaan.id })
				.then((response)=> {
				  var res = response.data;
