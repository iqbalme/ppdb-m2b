		...
		<div class="container-fluid">
			<div class="row">
				<div class="col-md-12">
						...
						<div class="row" v-if="!loading">
						<div class="col-md-12">
						<base-input type="text" label="NAMA SETTING" v-model="setting.setting" :validatedClass="$v.setting.$error" placeholder="Masukkan setting di sini"></base-input>
						<base-input type="text" label="VALUE" v-model="setting.value" :validatedClass="$v.setting.$error" placeholder="Masukkan value di sini"></base-input>
...
export default {
    ...
    data() {
        return {
			...
			data: [],
			kolom: ['no', 'setting', 'value', {key:'aksi', label:''}],
        };
    },
    ...
    methods: {
		...
		editData(id){
			this.setting.value = this.data[id].value;
			this.setting.setting = this.data[id].setting;
			this.setting.id = this.data[id].id;
		},
		hapusData(id){
			if(this.data.length > 0){
				...
				$axios
				.delete('/setting/' + this.data[id].id)
				.then((response)=> {
		...
		simpanData(){
			if(this.setting.value != null){
			  $axios
				.post('/setting', { 'setting' : this.setting.setting, 'id' : this.setting.id, 'value' : this.setting.value })
				.then((response)=> {
				  var res = response.data;
