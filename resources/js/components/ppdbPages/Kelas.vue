		...
		<div class="container-fluid">
			<div class="row">
				<div class="col-md-12">
						...
						<div class="row" v-if="!loading">
						<div class="col-md-12">
						<base-input type="text" label="" v-model="kelas.value" :validatedClass="$v.kelas.$error" placeholder="Masukkan kelas di sini"></base-input>
...
export default {
    ...
    methods: {
		...
		editData(id){
			this.kelas.value = this.data[id].kelas;
			this.kelas.id = this.data[id].id;
		},
		hapusData(id){
			if(this.data.length > 0){
				if(this.kelas.id != null){
					this.data.splice(id,1);
				}
				$axios
				.delete('/kelas/' + this.data[id].id)
				.then((response)=> {
		...
		simpanData(){
			if(this.kelas.value != null){
			  $axios
				.post('/kelas', { 'kelas' : this.kelas.value, 'id' : this.kelas.id })
				.then((response)=> {
				  var res = response.data;
